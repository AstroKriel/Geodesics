...
def _reconstruct_path(end_node: Node, vertices: List[Tuple[float, float, float]]) -> List[Tuple[float, float, float]]:
  path = []
  current = end_node
  while current:
...
def a_star_3d(
  ...
  while open_nodes:
    if use_priority_queue:
      ...
      current_node = min(open_nodes, key=lambda n: n.total_estimated_cost)
    ...
    for neighbor_vi in adjacency[current_node.vertex_index]:
      ...
      neighbor_node.total_estimated_cost = (
        neighbor_node.distance_traveled + heuristic_cost
      )
